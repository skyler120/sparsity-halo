...
parser.add_argument('--mcp', default=False, const=True, action='store_const', help='use mcp regularization')
parser.add_argument('--psi', default=1e-10, type=float, help='penalization coefficient on regularization coefficients')
parser.add_argument('--xi', default=1e-10, type=float, help='penalization coefficient on regularization coefficients')
...
def train():
    ...
    for batch_idx, (inputs, targets) in enumerate(trainloader):
        ...
        crit = args.psi * args.xi
        if args.mcp:
            for i, m in enumerate(model.modules()):
                if isinstance(m, nn.Conv2d): # or isinstance(m, nn.Linear):
                    for param in m.parameters():
                        opt1 = args.xi * torch.abs(param) - param**2 / (2 * args.psi)
                        opt2 = args.psi * args.xi**2 / 2.
                        reg_loss  = reg_loss + torch.sum(opt1 * torch.abs(param).le(crit) + opt2 * torch.abs(param).gt(crit))
        ...
        writer.add_scalar('train/top1_acc', top1.avg, global_step)
        writer.add_scalar('train/top5_acc', top5.avg, global_step)
...
if __name__ == '__main__':
    ...
    if args.evaluate:
        ...
        # Train and val
        title = 'runs/exp-ncvx-{}-{}-{}-{}'.format(args.dataset, args.arch, args.psi, args.xi)
